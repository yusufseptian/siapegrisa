	...
	<?php foreach ($matapelajaran as $mp) : ?>
		...
		<form method="post" action="<?php echo base_url('administrator/matapelajaran/update_aksi'); ?>">
			...
			<div class="form-group">
				...
				RIGHT JOIN tahun_ajaran ON(semester.id_ta = tahun_ajaran.id)')->result();
				?>
				<select name="id_semester" class="form-control">
					<option value="">Pilih Semester...</option>
					<?php foreach ($semester as $item) { ?>
						<option <?= ($item->id == $mp->id_semester) ? "selected" : '' ?> value="<?= $item->id_semester ?>"><?= $item->semester . ' - ' . $item->tahun_ajaran ?></option>
					<?php } ?>
				</select>
			</div>
